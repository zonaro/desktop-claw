...
const run = (...args: Array<string>) => {
  ...
  if (process.platform === 'darwin') {
    ...
    const exeName = `DesktopClaw${__DEV__ ? '-dev' : ''}.exe`
    ...
    execFile('/bin/desktop-claw', args, cb)
  } else {
...
const usage = (exitCode = 1): never => {
  process.stderr.write(
    'Desktop Claw CLI usage: \n' +
      '  desktop-claw-cli                           Open the current directory\n' +
      '  desktop-claw-cli open [path]               Open the provided path\n' +
      '  desktop-claw-cli clone [-b branch] <url>   Clone the repository by url or name/owner\n' +
