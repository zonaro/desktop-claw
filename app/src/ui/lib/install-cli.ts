...
export const InstalledCLIPath = '/usr/local/bin/desktop-claw-cli'

/** The path to the packaged CLI. */
const PackagedPath = Path.resolve(__dirname, 'static', 'desktop-claw-cli.sh')
  ...
  return new Promise<void>((resolve, reject) => {
    fsAdmin.unlink(InstalledCLIPath, error => {
      if (error !== null) {
        reject(
          new Error(
            `Failed to remove file at ${InstalledCLIPath}. Authorization of Desktop Claw Helper is required.`
          )
        )
        return
      }
